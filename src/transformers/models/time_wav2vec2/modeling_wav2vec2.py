...
from ..wav2vec2.modeling_wav2vec2 import Wav2Vec2Model
...
@dataclass
class TimeWav2Vec2ForDistillationOutput(ModelOutput):
    """
    Output type of [`TimeWav2Vec2ForDistillation`], with potential hidden states and attentions.
    ...
    contrastive_loss: Optional[torch.FloatTensor] = None
    diversity_loss: Optional[torch.FloatTensor] = None
    flow_matching_loss: Optional[torch.FloatTensor] = None
...
class Wav2Vec2Attention(nn.Module):
    ...
    def forward(
        ...
        sinusoidal_emb: torch.Tensor, # time embedding
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # get query proj
        query_states = self.q_proj(hidden_states, sinusoidal_emb).view(*q_input_shape).transpose(1, 2)
        ...
        if (
            ...
            # cross_attentions
            key_states = self.k_proj(key_value_states, sinusoidal_emb).view(*kv_input_shape).transpose(1, 2)
            value_states = self.v_proj(key_value_states, sinusoidal_emb).view(*kv_input_shape).transpose(1, 2)
        elif past_key_value is not None:
            # reuse k, v, self_attention
            key_states = self.k_proj(hidden_states, sinusoidal_emb).view(*kv_input_shape).transpose(1, 2)
            value_states = self.v_proj(hidden_states, sinusoidal_emb).view(*kv_input_shape).transpose(1, 2)
...
class TimeWav2Vec2ForDistillation(TimeWav2Vec2PreTrainedModel):
    def __init__(self, teacher_model: Wav2Vec2Model, config: TimeWav2Vec2Config):
        super().__init__(config)
        self.wav2vec2 = TimeWav2Vec2Model(config)
        self.dropout_features = nn.Dropout(config.feat_quantizer_dropout)
    ...
    def forward(
        ...
        loss = contrastive_loss = diversity_loss = flow_matching_loss = None
        if sampled_negative_indices is not None:
            batch_size, sequence_length, hidden_size = quantized_features.shape
            ...
            loss = contrastive_loss \
                + self.config.diversity_loss_weight * diversity_loss \
                + self.config.flow_matching_loss_weight * flow_matching_loss
